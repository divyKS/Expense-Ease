    ...
    async (c) => {
        ...
        async function fetchFinancialData( userId: string, startDate: Date, endDate: Date) {
            return await db
                ...
                .from(transactions) // transactions don't belong to user, they do to accounts, hence the inner join
        ...
        // to show the category wise spenditure
        const category = await db
            ...
            .from(transactions)
            .innerJoin(accounts, eq(transactions.accountId, accounts.id))
            .innerJoin(categories, eq(transactions.categoryId, categories.id)) // we dont want to have something that does not have a category hence we are doing IJ and not the LJ
            .where(
                and(
                    accountId ? eq(transactions.accountId, accountId) : undefined,
        ...
        // just active days are not going to be enough data points and will not help in a good graph
